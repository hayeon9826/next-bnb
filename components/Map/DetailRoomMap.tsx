...
export default function DetailRoomMap({ data }: { data: RoomType }) {
  const loadKakaoMap = () => {
    window.kakao.maps.load(() => {
      ...
      // 마커가 표시될 위치입니다
      let markerPosition = new window.kakao.maps.LatLng(data.lat, data.lng)

      let imageSrc = '/images/marker-shadow.png'
      let imageSize = new window.kakao.maps.Size(40, 40) // 마커이미지의 크기입니다
      let imageOption = { offset: new window.kakao.maps.Point(20, 60) } // 마커이미지의 옵션입니다. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정합니다.
      ...
      let content = `<div class="custom_overlay">${data.price?.toLocaleString()}원</div>`
